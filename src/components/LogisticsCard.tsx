...
type CandidateWithLogistics = import("@prisma/client").Candidate & {
  logistics: import("@prisma/client").LogisticsEvent[];
};

export default function LogisticsCard({ candidate }: { candidate: CandidateWithLogistics }) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    try {
      ...
      setFormMessage({ tone: "success", text: "Llegada registrada correctamente." });
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : "Error desconocido";
      setFormMessage({ tone: "error", text: `Error: ${msg}` });
    } finally {
            ...
            <div style={{ fontSize: "0.875rem", color: "var(--muted)", display: "flex", gap: "1rem", marginTop: "0.5rem" }}>
              ...
              {lastEvent ? (
                <span>
                  ...
                  {lastEvent.arrivalDate ? new Date(lastEvent.arrivalDate).toLocaleString("es-ES") : "N/A"}
                </span>
              ) : null}
            </div>
          ) : null}
        ...
        <div style={{ marginTop: "1rem", borderTop: "1px solid var(--grey-olive)", paddingTop: "1rem" }}>
          ...
          {lastEvent ? (
            <div style={{ backgroundColor: "var(--white-smoke)", padding: "1rem", border: "1px solid var(--pitch-black)", marginBottom: "1rem" }}>
              <p style={{ margin: "0 0 0.5rem 0", fontWeight: "bold" }}>Ultimo viaje registrado:</p>
              <p style={{ margin: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
                <Calendar size={16} /> {lastEvent.arrivalDate ? new Date(lastEvent.arrivalDate).toLocaleString("es-ES") : "N/A"}
              </p>
              <p style={{ margin: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
                <Car size={16} /> {lastEvent.transportType}
              </p>
              {lastEvent.terminal ? (
                <p style={{ margin: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  <MapPin size={16} /> Terminal: {lastEvent.terminal}
                </p>
              ) : null}
              {lastEvent.pickedUpBy ? (
                <p style={{ margin: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  <Clock size={16} /> Recoge: {lastEvent.pickedUpBy}
                </p>
              ) : null}
            </div>
          ) : (
          ...
          <form action={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            ...
            <label className="label" style={{ marginBottom: 0 }}>Terminal / Estacion</label>
